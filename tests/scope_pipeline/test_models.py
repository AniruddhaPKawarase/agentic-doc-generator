...
# ---------------------------------------------------------------------------
# New field tests — Task 2: ScopeItem extensions
# ---------------------------------------------------------------------------

def test_scope_item_new_fields_defaults():
    from scope_pipeline.models import ScopeItem
    item = ScopeItem(text="Install conduit", drawing_name="E-201")
...
def test_scope_item_new_fields_populated():
    from scope_pipeline.models import ScopeItem
    item = ScopeItem(
        text="Install conduit",
        drawing_name="E-201",
        drawing_refs=["E-201", "E-202"],
        discipline="ELECTRICAL",
        source_type="specification",
        csi_division="Division 26 — Electrical",
    )
    assert item.drawing_refs == ["E-201", "E-202"]
    assert item.discipline == "ELECTRICAL"
    assert item.source_type == "specification"
...
def test_classified_item_trade_color():
    ...
    assert item.trade_color == "#FFD700"
    # New ScopeItem fields should also be accessible with their defaults
    assert item.drawing_refs == []
    assert item.discipline is None
    assert item.source_type == "drawing"
...
def test_classified_item_trade_color_default():
    ...
    item = ClassifiedItem(
        ...
        classification_reason="Raceway falls under Division 26",
    )
